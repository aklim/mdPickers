...
function TimePickerCtrl($scope, $mdDialog, time, autoSwitch, $mdMedia) {
    ...
    this.cancel = function () {
        $mdDialog.cancel(this.time.toDate());
    };
        ...
        link:             function (scope, element, attrs, ctrl) {
            ...
            var onEvent = function (event) {
                ...
                $timeout(function () {
                    ...
                    if (ctrl.autoSwitch && ["mouseup", "click"].indexOf(event.type) !== -1 &&
                        timepickerCtrl) {
        ...
        var timePicker = function (time, options) {
            ...
            return $mdDialog.show({
                ...
                template:            '<md-dialog aria-label="" class="mdp-timepicker" ng-class="{ \'portrait\': !$mdMedia(\'gt-xs\') }">' +
                                     ...
                                     '<md-dialog-actions layout="row">' +
                                     '<span flex></span>' +
                                     '<md-button ng-click="timepicker.cancel()" aria-label="' + LABEL_CANCEL + '">' +
                                     LABEL_CANCEL + '</md-button>' +
                                     '<md-button ng-click="timepicker.confirm()" class="md-primary" aria-label="' +
                                     LABEL_OK + '">' + LABEL_OK + '</md-button>' +
                                     '</md-dialog-actions>' +
                                     '</div>' +
                                     '</md-dialog-content>' +
                                     '</md-dialog>',
...
module.directive("mdpTimePicker", ["$mdpTimePicker", "$timeout", function ($mdpTimePicker, $timeout) {
    return {
        ...
        link:       function (scope, element, attrs, ngModel, $transclude) {
            ...
            ngModel.$validators.format = function (modelValue, viewValue) {
                return !viewValue || angular.isDate(viewValue) || moment(viewValue, scope.timeFormat, true)
                        .isValid();
            };

            ngModel.$validators.required = function (modelValue, viewValue) {
                return angular.isUndefined(attrs.required) || !ngModel.$isEmpty(modelValue) ||
                    !ngModel.$isEmpty(viewValue);
            };
        ...
        link:     function (scope, element, attrs, ngModel, $transclude) {
            ...
            function showPicker(ev) {
                $mdpTimePicker(ngModel.$modelValue, {
                    ...
                    .then(function (time) {
                        ngModel.$setViewValue(moment(time)
                            .format(scope.format));
                        ngModel.$render();
                    });
            }
